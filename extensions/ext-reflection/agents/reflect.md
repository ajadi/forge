...
After appending the Reflect section to the task file, also record durable findings in memory:
- If a proposal is a lasting framework/process change, append a one-line entry to `memory/decisions.md` under a dated heading.
- If a recurring pattern was identified, append it to `memory/patterns.md` tagged `[recurring]`.

Proposals in the task file are always preserved regardless. Keep memory entries to one line each.
